...
class TradingSimulator:
    def __init__(
        ...
        self.data = yf.download(
            assets, start=compute_date, end=end_date, group_by="ticker"
        )
        if len(assets) == 1:
        ...
        for stock in assets:
            df[(stock, "MA")] = (
                df[(stock, "Adj Close")].rolling(window=28).apply(self.__MA)
            )
    ...
    def step(self, action):
        ...
        for i in range(len(self.portfolio)):
            ...
            self.portfolio[i]["num_shares"] = (
                weight_adjusted_stock_value / self.portfolio[i]["price"]
            )
        ...
        for i in range(len(self.portfolio) - 1):
            total_tx_cost += (
                abs(self.portfolio[i]["num_shares"] - old_portfolio[i]["num_shares"])
                * self.tx_fee
            )
        ...
        if self.time == np.ceil(
            len(self.close_price) / self.rebalance_window
        ):  # The episode is ended
            done = 1
            new_state = np.zeros(
                (4, len(self.assets), self.rebalance_window, len(self.assets))
            )
