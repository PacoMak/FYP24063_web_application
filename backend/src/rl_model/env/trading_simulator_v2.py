...
class TradingSimulatorV2:
    def __init__(
        ...
        for stock in assets:
            ...
            df[(stock, "Volume")] = df[(stock, "Volume")].apply(
                lambda x: x / (10**max_vol_scale)
            )
            df[(stock, "OBV")] = (
                np.sign(df[(stock, "Returns")]) * df[(stock, "Volume")]
            ).cumsum()
            ...
            df[(stock, "DevToMidBand")] = (
                df[(stock, "Close")] - df[(stock, "MA20")]
            ) / df[(stock, "Std")]
            df.loc[0, (stock, "EMA_12")] = df.loc[0, (stock, "Close")]
            df.loc[0, (stock, "EMA_26")] = df.loc[0, (stock, "Close")]
            for i in range(1, len(df)):
                df.loc[i, (stock, "EMA_12")] = self.__EMA(
                    12, df.loc[i, (stock, "Close")], df.loc[i - 1, (stock, "EMA_12")]
                )
                df.loc[i, (stock, "EMA_26")] = self.__EMA(
                    26, df.loc[i, (stock, "Close")], df.loc[i - 1, (stock, "EMA_26")]
                )

            df[(stock, "MACD")] = df[(stock, "EMA_12")] - df[(stock, "EMA_26")]
            df.loc[0, (stock, "signal")] = df.loc[0, (stock, "MACD")]
            for i in range(1, len(df)):
                df.loc[i, (stock, "signal")] = self.__EMA(
                    9, df.loc[i, (stock, "MACD")], df.loc[i - 1, (stock, "signal")]
                )
        ...
        close_data = close_data[start_index - 1 :].reset_index(
            drop=True
        )  # Take an extra day before the first in-range trading date
        ...
        treasury_rate = read_treasury_rates(self.file_path)
        ...
        dev_data = close_data.loc[
            :, close_data.columns.get_level_values(1) == "DevToMidBand"
        ]
        ...
        signal_data = close_data.loc[
            :, close_data.columns.get_level_values(1) == "signal"
        ]
        ...
        self.risk_free_rates = pd.read_csv(self.file_path)
        self.risk_free_rates["date"] = pd.to_datetime(
            self.risk_free_rates["date"]
        ).dt.year
    ...
    def calculate_tangent_portfolio(
        self, expected_returns, covariance_matrix, risk_free_rate
    ):
        ...
        def objective_function(weights):
            ...
            sharpe_ratio = (portfolio_return - risk_free_rate) / np.sqrt(
                portfolio_variance
            )
            return -sharpe_ratio
    ...
    def sharpe_ratio(self):
        ...
        risk_free_rates = pd.read_csv(self.file_path)
    ...
    def omega_ratio(self, target_rate):
        window_target_rate = (1 + target_rate / 100) ** (
            1 / (252 / self.rebalance_window)
        ) - 1
        sorted_portfolio_returns = np.sort(
            pd.Series(self.value_history).pct_change().dropna()
        )
        cdf = np.arange(1, len(sorted_portfolio_returns) + 1) / len(
            sorted_portfolio_returns
        )
        cdf_value_at_k = np.interp(
            window_target_rate, sorted_portfolio_returns, cdf
        )  # Interpolate to find CDF value at k
    ...
    def avg_monthly_return(self):
        ...
        monthly_return_rates = (
            portfolio_history.groupby(portfolio_history["date"].dt.to_period("M"))[
                "value"
            ]
            .apply(self.period_return)
            .reset_index()
    ...
    def get_tangent_weights(self, lag):
        ...
        risk_free_rate = self.risk_free_rates[self.risk_free_rates["year"] == d.year][
            "risk_free_rate"
        ].values[0]
    ...
    def step(self, action):
        ...
        if len(action) != 0:
            for i in range(len(self.portfolio)):
                ...
                self.portfolio[i].set_num_shares(
                    weight_adjusted_stock_value / self.portfolio[i].get_price()
                )
                self.portfolio[i].set_value(weight_adjusted_stock_value)
        else:
            for i in range(len(self.portfolio)):
                self.portfolio[i].set_weighting(
                    self.portfolio[i].get_value() / new_value
                )
